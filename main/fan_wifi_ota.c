...
ESP_EVENT_DEFINE_BASE(FAN_OTA_EVENT);
...
esp_err_t ota_post_handler(httpd_req_t *req) {
    ...
    while (remaining > 0) {
        ...
        if (image_header_was_checked == false) {
            ...
            if (data_read > sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t) + sizeof(esp_app_desc_t)) {
                ...
                common_post_event(FAN_OTA_EVENT, OTA_START);
            } else {
                ...
                common_post_event(FAN_OTA_EVENT, OTA_FAILED);
                return ESP_FAIL;
            }
        }
        ...
        common_post_event_data(FAN_OTA_EVENT, OTA_PROGRESS, &upgrade_progress, sizeof(float *));
    }
    ...
    common_post_event(FAN_OTA_EVENT, OTA_SUCCESS);
